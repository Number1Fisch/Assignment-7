...
function getAllParagraphs(){
    pList = document.querySelectorAll(document.getElementsByName('myPage'));
    return pList;
}
...
function makeItalic(){
    pList = getAllParagraphs();
    newList;
    for(i = 0; i < length(pList); i ++){
        if (pList[i].classList.contains('even')){
            pList[i].style.fontStyle = 'italic';
...
function createBorders(){
   table = getTheTable();
   table.style.createBorders = "2px solid #000";
   return table;
}
...
function getTheListItem(num1, num2){
    page = document.getElementsByName('myPage');
    list = page.querySelectorAll('ul');
...
function changeBgColor(color){
    page = document.getElementsByName('myPage');
    document.page.style.backgroundColor = color;
    return page;
}